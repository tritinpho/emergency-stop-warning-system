...
|---|---|---|---|---|
| 1 | Detector class set | `deploy_config` declares single class `"vehicle"`, but `main.py` **refuses to load it** and always falls back to 80-class COCO `yolov8n_320` | per-class footprint car/truck/bus + person (SC-12) | **resolved** — target COCO (the device already does). Single-class is a *degraded* mode surfaced by `model_capabilities()`, never silent. The custom kmodels are **dead**; day/night switching is **inert**. New gap: the model that runs is **unpinned** — no SHA, in no repo ([models/README.md](models/README.md)) |
...
| 4 | Shake mitigation | `ShakingFilter` = MicroPython no-op | real stabilisation (field) | **do not count** as on-device mitigation. Our actual jitter mitigation is the windowed + EMA speed estimator in `esw.perception` (PC-06) |
| 4a | Traffic density | `OverVehiclesFilter` gates *tracking* at `>2 vehicles` OR `≥0.4 occupancy` | R14 congestion suppression of the *sign* | **harvested** (2026-07-10) — signal adopted (`n_vehicles` + frame occupancy, measured pre-association), thresholds rejected: suppression is fail-danger, so ours needs **both**, bounded low. SC-44/45/46/47, PC-12, AP-12. See [ADR-0016](../../docs/adr/ADR-0016-repo-consolidation-and-perception-source.md) AI#3 |
| 4b | Lighting noise | `LightFilter` computed, then **never fed to the detector** (`main.py` infers on the raw frame) | validated night preprocessing | **seam only** — `ESW_LIGHT_FILTER = False` in `esw-app/main.py`. Enabling it is an A/B question for `tools/score_capture.py`, not a naming decision |
