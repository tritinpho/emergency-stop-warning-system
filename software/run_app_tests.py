...
from harness.devices import (FakeClock, ListCapture, RamStore, ScriptedCommands,
                             ScriptedDetector, Selftest, SignLink)
...
TICK_DT = 0.1
_SITE = "bench-01"
_KEY = crypto.derive_key(b"esw-master-secret-v1-0123456789abc", "IF4", _SITE)
_VERSIONS = {"fw_ver": "bench-fw", "model_ver": "yolov8n_320-unpinned", "calib_ver": "bench-calib"}


def drive(case, store=None, transport=None, app=None):
    ...
    sign = Sign(default_config(), _KEY, can_turn_off=not case.get("sign_stuck", False))
    link = SignLink(sign)
    cap = ListCapture()
    ...
    # would stop discriminating. AP-10 passes the file-backed store that survives a power loss.
    ...
    if app is None:
        app = EdgeApp(_KEY, _SITE, _VERSIONS, CALIB, backends)
    boot = app.start()
...
def evidence_checks():
    ...
    try:
        ...
        transport.up = True
        app2 = EdgeApp(_KEY, _SITE, _VERSIONS, CALIB,
                       {"detector": ScriptedDetector(EVIDENCE_CASE, yolo_frame, LABELS),
                        "radio": SignLink(Sign(default_config(), _KEY)),
