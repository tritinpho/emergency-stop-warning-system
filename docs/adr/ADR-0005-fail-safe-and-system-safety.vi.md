...
3. [ ] Hiện thực **cơ chế an toàn tự kích hoạt khi mất tín hiệu (dead-man's switch)**: cơ cấu chấp hành mặc định về trạng thái để trống khi mất nhịp tín hiệu khẳng định (assertion) từ máy trạng thái, cộng với một đường buộc-về-an-toàn độc lập từ bộ giám sát tình trạng → cơ cấu chấp hành (kiểm chứng bằng cách giết tiến trình máy trạng thái trong một bài kiểm thử tiêm lỗi và xác nhận bảng báo chuyển về để trống).
