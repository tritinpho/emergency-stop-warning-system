...
2. **Dữ liệu mang tính đại diện đến từ đâu?** Lựa chọn nhận diện ([ADR-0003](ADR-0003-detection-algorithm.vi.md)) cần các đoạn clip ngày/đêm/mưa của lề đường cao tốc Việt Nam để tinh chỉnh và đánh giá — và việc thu thập video bên đường làm phát sinh chính những nghĩa vụ về quyền riêng tư ([tài liệu 04 §4](../04-risk-and-safety.vi.md#4-quyền-riêng-tư-dữ-liệu--tuân-thủ-pháp-lý)) ngay ở giai đoạn **nguyên mẫu (prototype)**, chứ không chỉ ở hiện trường.
...
- **Giàn thử nghiệm** kiểm chứng phần *nhận diện + tác động (actuation)*: một camera thật (**chỉ dùng camera — radar đã không được cấp kinh phí**, [ADR-0001](ADR-0001-sensing-modality.vi.md) bị Bác bỏ 2026-07-10) điều khiển vòng lặp thật để ra lệnh cho một bảng báo thay thế (stand-in sign) trên các kịch bản vật lý được dàn dựng.

**Ranh giới chứng minh được (nêu rõ trong báo cáo).** Giàn thử/mô phỏng *có thể* tuyên bố: tính đúng đắn của logic, định thời/độ trễ (latency), xử lý lỗi, và khả năng kháng kích hoạt sai đối với các nhiễu (nuisance) *đã được mô hình hóa*. Chúng **không thể** tuyên bố: độ nhạy phát hiện (recall) trong điều kiện thực khi mưa/lóa sáng/sương mù, tỷ lệ báo động giả thực, hiệu năng trước nhiễu radar thực, hay **tính đúng đắn trong thực tế của cơ chế giữ-khi-che-khuất có radar củng cố** — những điều này được **hoãn sang giai đoạn hiện trường (field-deferred)** cho lần thử nghiệm cấp sở ([tài liệu 05](../05-field-pilot-proposal.vi.md)).

**Điểm cần lưu ý về giữ-khi-che-khuất sắc hơn "recall bị hoãn".** Mô phỏng kiểm chứng *chính sách* của [ADR-0008](ADR-0008-detection-persistence-and-multitrack.vi.md) / [ADR-0009 §C](ADR-0009-failsafe-placement-and-degraded-modes.vi.md) **với các ngữ nghĩa cảm biến đúng mà nó được cấp** — nhưng *tính an toàn* của chính sách lại phụ thuộc vào việc radar thật phân biệt được lề đường với làn xuyên suốt ở cự ly giám sát (tiêu chí (b) của cổng [ADR-0001](ADR-0001-sensing-modality.vi.md)), điều mà một bàn thử vài mét **không thể tái tạo** (đó là một bài toán góc — xem ADR-0001) và bị hoãn sang đường thử/hiện trường. Vì vậy báo cáo có thể nói logic giữ-khi-che-khuất / `CAMERA_OCCLUDED_DEGRADED` là *đúng như đặc tả*, **chứ không** phải *đúng đắn trong thực tế*; nếu (b) yếu thì chính logic đó đảo một bỏ sót thầm lặng thành một **kẹt-BẬT** ([tài liệu 04 R12/R14](../04-risk-and-safety.vi.md#1-bảng-đăng-ký-rủi-ro)).

**Các kênh cảm biến tổng hợp (synthetic sensor channels)** được phép sử dụng nhưng phải dùng một **mô hình cảm biến được lập tài liệu, thận trọng (conservative)** với các giả định được nêu rõ; một radar tổng hợp *giả định* khả năng phát hiện xe đứng yên hoàn hảo thì không thể được dùng để làm bằng chứng cho độ nhạy phát hiện trong điều kiện bất lợi (xem [ADR-0001](ADR-0001-sensing-modality.vi.md) và cách phân tách nghiệm thu ở [tài liệu 01 §5](../01-requirements.vi.md#5-chỉ-số-đánh-giá--tiêu-chí-nghiệm-thu)).
...
**Tiêu chí đạt (mô phỏng).** Vòng lặp kín đáp ứng các mục tiêu ở *cột nguyên mẫu (prototype-column)* của tài liệu 01 §5 trên toàn bộ danh mục kịch bản, việc tiêm lỗi bắt được ≥ 95 % danh sách FMEA ([tài liệu 04 §2](../04-risk-and-safety.vi.md#2-fmea-lite-kiểu-lỗi--tác-động--phát-hiện--phản-ứng)), và **không có lỗi nào được tiêm vào tạo ra một đầu ra gây nhầm lẫn hoặc bị kẹt**.
...
1. [ ] Viết **phương pháp luận mô phỏng** — danh mục kịch bản, mô hình cảm biến tổng hợp + các giả định (gồm cả hành vi nhiễu / mất tín hiệu (dropout) / che khuất), quy tắc gán nhãn chân trị (ground-truth), và tiêu chí đạt — và **đóng băng nó như một sản phẩm của Giai đoạn 2**, không phải một sản phẩm phụ của Giai đoạn 3: nó là nền tảng mà mọi tuyên bố về logic của Giai đoạn 3 đặt lên trên, nên phải được chốt *trước khi* vòng lặp được xây dựng ([tài liệu 03 §3](../03-roadmap-and-phasing.vi.md#3-kế-hoạch-giai-đoạn-khớp-với-6-giai-đoạn-của-đề-xuất)).
...
3a. [ ] **Định cỡ việc tạo bằng chứng nghiệm thu** như một sản phẩm bàn giao của Giai đoạn 1: độ nhạy phát hiện (recall) N ở [§5](../01-requirements.vi.md#5-chỉ-số-đánh-giá--tiêu-chí-nghiệm-thu) phải là **các lần thu thập THẬT** (N tổng hợp không được tính vào recall) và tỷ lệ kích hoạt sai theo giờ cần **số giờ chạy bàn thử liên tục** — vì vậy hãy đặt một số mục tiêu cho số sự kiện dương (ví dụ ≥ 200, gồm cả ban đêm) và một quy trình dàn dựng-và-thu-thập có thể đạt tới giới hạn Wilson, được lên lịch và cấp nguồn lực như mũi nhọn (spike) radar. Các tập dữ liệu công khai rất thưa các trường hợp dương về xe dừng trên lề, nên phần lớn các trường hợp dương thật sẽ được **dàn dựng**; hãy lên kế hoạch hậu cần ngay bây giờ, chứ không phải đến Giai đoạn 5.
...
5. [ ] Ghi lại **phân tách bối cảnh** của cổng radar — (a) phát hiện xe đứng yên trong nhiễu nền là bàn thử/Giai đoạn 3; (b) phân biệt phương vị/làn ở cự ly giám sát là đường thử/hiện trường — và gắn nhãn các bảo đảm giữ-khi-che-khuất / `CAMERA_OCCLUDED_DEGRADED` là **được thiết kế (logic đã kiểm chứng), chưa đúng đắn-thực-tế**, cho đến khi (b) đạt.
