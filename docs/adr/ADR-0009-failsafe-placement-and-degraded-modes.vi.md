...
| **RADAR-ONLY** | camera chết | **Không** — không có lớp, không có cổng ROI trên ảnh; phương vị radar có thể không phân giải được lề so với làn thông (through-lane) | Chỉ được giữ **dưới dạng trạng thái giữ khi camera không xác thực được (vết) có giới hạn** — `T_degraded_max` → buộc xóa lớn tiếng, **không** bắt lại (vết) ([ADR-0013](ADR-0013-degraded-hold-unification.vi.md)) | **BLIND-TO-NEW: cảnh báo nghiêm trọng** — *không phải* một lần chạy lành tính |
| **NEITHER** | cả hai hỏng | Không | Không | **SAFE STATE** (để trống) + cảnh báo nghiêm trọng |

Phần chỉnh sửa chịu lực là **RADAR-ONLY**: nó **mù với các mối nguy mới (blind to new hazards)** và phải leo thang như một suy giảm **nghiêm trọng (critical)**, trong khi các vết đã được xác nhận chỉ có thể tồn tại dưới dạng **trạng thái giữ khi camera không xác thực được (vết) có giới hạn** — *cùng* một giới hạn `T_degraded_max` như khi che khuất kéo dài, nên một cảnh báo đang diễn ra không bị rớt ngay khoảnh khắc camera hỏng **nhưng cũng không thể kẹt BẬT mãi mãi** trên một phát hiện radar mà camera đã chết không còn xác thực được. Cái "khoảng giữ ngắn" này đã bị để vô hạn trong bản phác thảo đầu tiên của ADR này; **[ADR-0013](ADR-0013-degraded-hold-unification.vi.md)** khép nó lại bằng cách làm cho giới hạn `T_degraded_max` trở nên bất-kể-nguyên-nhân (che khuất *hoặc* lỗi camera) và liệt kê đầy đủ ma trận trạng thái cảnh báo × chế độ cảm biến. Một đơn vị không thể xác nhận một lần dừng mới **không bao giờ được tự thể hiện mình như đang giám sát**. (CAMERA-ONLY vẫn là một lần-chạy-suy-giảm đích thực, vì chỉ riêng camera vẫn có thể khởi tạo; nó chỉ đơn thuần mất đi tính bền vững trước thời tiết và khoảng giữ-khi-che-khuất của radar.)
