...
- bảo đảm phát hiện vượt ra ngoài phạm vi đã kiểm chứng. **Ban đêm, mưa và sương mù nằm ngoài phạm vi
  đó.** [ADR-0001](adr/ADR-0001-sensing-modality.vi.md) (camera + radar) đã bị **Bác bỏ cho giai đoạn
  này** và hệ thống **chỉ dùng camera**, nên tuyên bố về điều kiện bất lợi **hoàn toàn không được đưa
  ra** — chứ không phải chỉ là chưa chứng minh (R5, không còn giảm thiểu);
- **giữ cảnh báo xuyên qua một lần che khuất.** Không có radar chứng thực, một xe đã xác nhận bị xe tải
  làn thông hành che khuất lâu hơn `T_hold` (mặc định 10 giây) sẽ bị **xóa** — biển báo tắt trong khi
  xe vẫn đang dừng. Việc xóa là *lớn tiếng* (người trực được cảnh báo), nên đây là một khoảng trống
  có-người-trực-trong-vòng-lặp, không phải một khoảng trống thầm lặng (R20);
- **báo cáo tình trạng của chính nó một cách có ý nghĩa.** Thiết bị chỉ-dùng-camera nằm vĩnh viễn ở chế
  độ `CAMERA_ONLY` nên luôn hiển thị trạng thái `DEGRADED`. Trong giai đoạn này `DEGRADED` nghĩa là
  *"radar chưa từng được lắp"*, **không phải** *"có thứ gì đó đã hỏng"* (R21).
...
| R5 | **Mù trong điều kiện bất lợi** — đêm/mưa/sương mù vô hiệu hóa camera | 4 | 4 | 16 | **Không còn biện pháp giảm thiểu trong giai đoạn này — phần dư được chấp nhận.** Radar là biện pháp duy nhất và đã bị **[Bác bỏ 2026-07-10](adr/ADR-0001-sensing-modality.vi.md)**; hệ thống chỉ dùng camera. Khả năng phát hiện ban đêm/mưa/sương mù **không được tuyên bố** — không đo được, cũng không suy ra từ radar tổng hợp. Cảnh báo chế độ suy giảm vẫn giữ. Biện pháp giảm thiểu hoãn sang đề tài hiện trường cấp sở. |
...
| R15 | **Lỗi hiệu chuẩn / trôi hiệu chuẩn** — homography sai hoặc thông số ngoại lai camera↔radar sai, hoặc trôi do cột rung lắc / chấn động / nhiệt, làm dịch chuyển ROI một cách thầm lặng → bỏ sót hoặc báo động giả có hệ thống | 3 | 4 | 12 | Quy trình hiệu chuẩn theo từng vị trí; kiểm tra lại định kỳ; **bộ giám sát trôi** — nay là một **hàm tự-giám-sát FR-10 có tên** với một đặc tả phần-dư-tham-chiếu-so-với-dung-sai được định nghĩa ([doc 01 FR-10](01-requirements.vi.md#2-yêu-cầu-chức-năng), [doc 02 §4](02-system-architecture.vi.md#4-máy-trạng-thái-phát-hiệncảnh-báo)); cảnh báo khi vượt dung sai. **Tầng:** *logic* phát hiện là kiểm-chứng-được-trên-bàn-thử (tiêm một phép dịch homography tổng hợp); trôi **thực** là hoãn-hiện-trường, nên ở phạm vi bàn thử biện pháp này là được-kiểm-chứng-về-logic, không phải đã-chứng-minh-ở-hiện-trường ([doc 01 §3a](01-requirements.vi.md#3a-phạm-vi-kiểm-chứng--giai-đoạn-được-tài-trợ-bàn-thửmô-phỏng-thực-sự-có-thể-chứng-minh-điều-gì)). |
...
| R18 | **Kẹt-BẬT trong một trạng thái suy giảm vượt qua mọi giới hạn tự động** — `CAMERA_OCCLUDED_DEGRADED` giữ bảng BẬT chỉ dựa trên radar trong khi watchdog bị vô hiệu hóa; camera có thể **bị che khuất _hoặc_ bị lỗi** | 2 | 4 | 8 | Giới hạn cuối **`T_degraded_max`** → buộc xóa lớn-tiếng độ-tin-cậy-thấp + leo thang mức cao nhất; **giới hạn này không phụ thuộc nguyên nhân — che khuất *hoặc* lỗi camera** ([ADR-0013](adr/ADR-0013-degraded-hold-unification.vi.md)), khép lại "khoảng giữ ngắn" CHỈ-RADAR trước đây không có giới hạn; NFR-04 được mở rộng để bao quát kẹt-BẬT do phân biệt-cảm-biến ([tài liệu 02 §4](02-system-architecture.vi.md#4-máy-trạng-thái-phát-hiệncảnh-báo), [ADR-0009 §C](adr/ADR-0009-failsafe-placement-and-degraded-modes.vi.md)). |
| R19 | **Không khớp về phạm vi/kỳ vọng SXTN** — loại hình tài trợ được nêu (*sản xuất thử nghiệm* / experimental-pilot-**production**) có thể ngụ ý theo hợp đồng về một *đơn vị sản xuất thử*, trong khi mức ngân sách 20M VND chỉ đủ tài trợ một nguyên mẫu trên bàn; khác với R11 (đây là **đơn vị tài trợ kỳ vọng nhiều hơn**, không phải nhóm vượt phạm vi) | 2 | 4 | 8 | Giải quyết loại-hình-dự-án so với sản-phẩm-bàn-giao **minh thị với đơn vị tài trợ ngay bây giờ** ([doc 03 §1](03-roadmap-and-phasing.vi.md#1-rà-soát-thực-tế-phạm-vi--ngân-sách-đọc-trước), [doc 00 bảng thuật ngữ](00-context-and-glossary.vi.md)): xác nhận sản phẩm cấp-trường là một nguyên mẫu nguyên lý, hoặc nêu sự không khớp về phạm vi/ngân sách **trước khi** các mốc hợp đồng ràng buộc — chứ không phải lúc rà soát cuối cùng. |

| R20 | **Che khuất làm xóa cảnh báo (bản chỉ-dùng-camera)** — khi [ADR-0001](adr/ADR-0001-sensing-modality.vi.md) đã bị Bác bỏ thì không có radar chứng thực, nên `corr` không bao giờ đúng: `WARN_HOLD` / `CAMERA_OCCLUDED_DEGRADED` **không bao giờ đạt tới được**, và một xe đã xác nhận bị xe tải làn thông hành che khuất sẽ bị **xóa sau `T_hold` (mặc định 10 giây)** — biển báo tắt trong khi xe vẫn đang dừng | 3 | 4 | 12 | **Chấp nhận cho giai đoạn này**; nêu rõ như một giới hạn bảo vệ (§0). Việc xóa là *lớn tiếng* (người trực được cảnh báo, sự kiện được ghi log), nên phần dư là **có-người-trực-trong-vòng-lặp, không thầm lặng**. Radar chứng thực hoãn sang cấp sở. **Không** thay thế bằng một "khoảng giữ" chỉ-dùng-camera nếu chưa quyết định đánh đổi cry-wolf trước — một khoảng giữ không được chứng thực chỉ là phỏng đoán, và R14 đã cho thấy đề tài này ưu tiên xóa lặng lẽ hơn là khẳng định sai. |
| R21 | **Trạng thái DEGRADED vĩnh viễn** — thiết bị chỉ-dùng-camera không bao giờ đạt chế độ cảm biến `FULL`, nên cảnh báo cho người trực (khóa theo chế độ) luôn báo `DEGRADED` ở mọi nhịp; một chỉ báo lúc nào cũng bật thì không mang thông tin và tập cho người trực thói quen bỏ qua nó (một biến thể cảnh-báo-thường-trực của R17) | 3 | 3 | 9 | **Chấp nhận cho giai đoạn này.** Phạm vi bàn thử không có người trực, nên mức phơi nhiễm là hoãn lại chứ không phải đang hiện hữu. Trước bất kỳ triển khai có người trực nào, phải khóa lại mức nghiêm trọng để phân biệt *chỉ-dùng-camera-theo-thiết-kế* với *camera-đã-hỏng*. Cho tới lúc đó, `DEGRADED` nghĩa là "radar chưa từng được lắp", **không phải** "có thứ gì đó đã hỏng". |

**Các mức phơi nhiễm cao nhất cần thiết kế đối phó trước tiên:** R5 (mù trong điều kiện bất lợi), R1 (bỏ sót thầm lặng), R4
(đặt quá gần). R1 và R4 được xử lý bằng các quyết định mang tính chịu lực đã được đưa ra (ADR-0005,
doc 01 §4). **R5 nay _không còn biện pháp giảm thiểu_:** biện pháp duy nhất của nó — hợp nhất camera+radar
ADR-0001 — đã bị **Bác bỏ cho giai đoạn này (2026-07-10)**. R5 được mang theo như một phần dư được chấp
nhận và hoãn sang đề tài cấp sở, cùng với hai hệ quả mà nó tạo ra, **R20** và **R21**.

## 2. FMEA-lite (kiểu lỗi → tác động → phát hiện → phản ứng)

| Kiểu lỗi | Tác động | Cách phát hiện | Phản ứng của hệ thống |
|--------------|--------|--------------|-----------------|
| Camera chết / khung hình đóng băng | Mất lớp phân loại + hình học ROI ảnh → **không thể xác nhận một xe dừng _mới_**; một cảnh báo đang hoạt động nay không được camera xác thực | Watchdog phát hiện khung hình cũ; kiểm tra tình trạng theo từng cảm biến | **CHỈ-RADAR = MÙ VỚI SỰ CỐ MỚI** ([ADR-0009 §B](adr/ADR-0009-failsafe-placement-and-degraded-modes.vi.md)): có thể *giữ* một cảnh báo đã được xác nhận từ trước **chỉ dưới dạng trạng thái giữ khi camera không xác thực được (vết) có giới hạn** — `T_degraded_max` → buộc xóa lớn tiếng, **không** bắt lại ([ADR-0013](adr/ADR-0013-degraded-hold-unification.vi.md)) — nhưng **không thể khởi tạo** → **cảnh báo trọng yếu (critical)**, *không phải* một "lần chạy suy giảm" lành tính; cả hai cảm biến đều hỏng → TRẠNG THÁI AN TOÀN |
...
- **Không giữ-suy-giảm vô hạn:** watchdog cố ý bị vô hiệu hóa bởi đối chứng bằng radar, nên
  ...
  **lớn tiếng** (xóa độ-tin-cậy-thấp + leo thang), không bao giờ giữ BẬT mãi dựa trên một tín hiệu radar
  không kiểm chứng được ([ADR-0009 §C](adr/ADR-0009-failsafe-placement-and-degraded-modes.vi.md)). Giới hạn
  này **không phụ thuộc nguyên nhân — camera bị che khuất _hoặc_ bị lỗi** — nên khoảng giữ khi camera-chết
  (CHỈ-RADAR), trước đây là một "khoảng giữ ngắn" không có giới hạn, chính là cùng một trạng thái có giới
  hạn ([ADR-0013](adr/ADR-0013-degraded-hold-unification.vi.md)).
