...
| `range_m` | float | mét (radar) |
| `speed_kph` | float | km/h; có dấu theo hướng tới/rời |
| `sensor_source` | enum/flags | `camera·radar·fused` — cho máy trạng thái biết nguồn đối chứng |
...
| `subsystem_health[]` | danh sách | theo từng camera/radar/tính toán/liên kết/biển báo |
| `sensor_mode` | enum | `FULL·CAMERA-ONLY·RADAR-ONLY·NEITHER` ([ADR-0009 §B](adr/ADR-0009-failsafe-placement-and-degraded-modes.vi.md), ma trận [ADR-0013](adr/ADR-0013-degraded-hold-unification.vi.md)) |
