...
| FR-10 | Tự giám sát + nhịp tín hiệu | M | [0005](adr/ADR-0005-fail-safe-and-system-safety.vi.md) | B·S | Sức khỏe từng hệ con; nhịp định kỳ | Nhịp mang sức khỏe + vân tay phiên bản; lỗi được phát hiện |
| FR-11 | Trạng thái an toàn + báo khi lỗi | M | [0005](adr/ADR-0005-fail-safe-and-system-safety.vi.md)/[0009](adr/ADR-0009-failsafe-placement-and-degraded-modes.vi.md) | B·S | **Tiêm lỗi** (giết SM, giết hộp, cắt liên kết) | Biển trống trong `T_signhold` ở **mọi** trường hợp; báo người trực |
...
| FR-20 | Ràng buộc biên cấu hình | M | [0010](adr/ADR-0010-operator-override-and-manual-control.vi.md)/[0012](adr/ADR-0012-security-and-threat-model.vi.md) | B·S | **Cấu hình ngoài biên** (ROI/`T_dwell=900 s`/v.v.) bị từ chối/ghim | Tham số ngoài-tầm bị từ chối hoặc ghim; giữ tốt-cuối; có báo |
...
| NFR-04 | Không kẹt-BẬT (mọi lỗi) | [0005](adr/ADR-0005-fail-safe-and-system-safety.vi.md)/[0008](adr/ADR-0008-detection-persistence-and-multitrack.vi.md)/[0009](adr/ADR-0009-failsafe-placement-and-degraded-modes.vi.md) | B·S | Hết hạn watchdog; **buộc-xóa `T_degraded_max`**; logic kẹt | Không trạng thái nào giữ BẬT thiếu xác nhận quy-được-về-làn; xóa có giới hạn trong mọi trường hợp |
