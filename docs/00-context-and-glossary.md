...
| Degraded mode — *initiate* vs *hold* | (chế độ suy giảm) | What a unit can still do with one sensor down: a camera-dead unit can *hold* an existing warning but cannot *initiate* a new one (**blind to new hazards**). The *hold* is the **bounded camera-unverified hold** (`T_degraded_max`, [ADR-0013](adr/ADR-0013-degraded-hold-unification.md)); the full **warning × sensor-mode matrix** is in [doc 02 §4](02-system-architecture.md#4-the-detectionwarning-state-machine). |
...
| `T_degraded_max` — bounded degraded hold | (cận thời gian suy giảm) | The hard time bound on `CAMERA_OCCLUDED_DEGRADED` (camera **unable to verify the track — occluded _or_ faulted** — only radar corroborating) after which the machine forces a **loud** disposition instead of holding the sign ON indefinitely on an unverifiable radar return ([ADR-0009 §C](adr/ADR-0009-failsafe-placement-and-degraded-modes.md)); the bound is **cause-agnostic** ([ADR-0013](adr/ADR-0013-degraded-hold-unification.md)), closing the stale-ON paths the watchdog cannot — both sustained occlusion and the camera-fault (RADAR-ONLY) hold. |
| Operator concept of operations (ConOps) / alarm management | (quy trình vận hành & quản lý cảnh báo) | The staffed, bounded operator response path that "fail loud" depends on — per-condition expected action, severity, target response time, alarm dedup/prioritization, re-escalation on non-ack ([ADR-0011](adr/ADR-0011-operator-concept-and-alarm-management.md), NFR-15). |
| Threat model | (mô hình mối đe dọa) | The enumerated assets, actors, and attack surfaces (sign link, override, config/OTA, sensor denial) against which the NFR-09 "cannot be spoofed" claim is **scoped** ([ADR-0012](adr/ADR-0012-security-and-threat-model.md)). |
| Presence-onset (pedestrian) | (kích hoạt theo hiện diện) | A **person** warrant is triggered by *presence* in/beside the ROI (debounced), **not** the vehicle stationarity gate a walking occupant would fail ([ADR-0003](adr/ADR-0003-detection-algorithm.md)). |
