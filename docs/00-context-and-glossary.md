...
| `T_degraded_max` — bounded degraded hold | (cận thời gian suy giảm) | The hard time bound on `CAMERA_OCCLUDED_DEGRADED` (camera occluded, only radar corroborating) after which the machine forces a **loud** disposition instead of holding the sign ON indefinitely on an unverifiable radar return ([ADR-0009 §C](adr/ADR-0009-failsafe-placement-and-degraded-modes.md)); closes the one stale-ON path the watchdog cannot. |
