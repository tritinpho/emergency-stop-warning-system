...
|-------|----------------|-----------|
| Bộ tính toán biên | **Kendryte K230** (RISC-V + KPU NPU, CanMV/MicroPython) — do nhóm phần cứng chọn | NPU trên chip chạy một bộ phát hiện nhỏ ở công suất thấp, thân thiện pin mặt trời; bộ phát hiện chuyển sang `kmodel` ([ADR-0003](adr/ADR-0003-detection-algorithm.vi.md)); vòng lặp an toàn theo [ADR-0015](adr/ADR-0015-state-machine-implementation-strategy.vi.md). |
