...
|----|---------------------|--------|---------------------------------|-------------------|
| ~~**RQ-H1**~~ | **CLOSED — NOT PROCURED (2026-07-10).** This phase is **camera-only**. Retained as the **cấp sở** specification, not a live ask: ~~radar that detects a *stationary* vehicle in roadside clutter (a) _and_ resolves shoulder vs. adjacent through-lane at the monitored range (b) — an imaging / HRR FMCW eval module, **not** a generic presence unit.~~ | [ADR-0001](adr/ADR-0001-sensing-modality.md) **Rejected** | **Realised, and accepted:** the occlusion hold never engages (**R20**); the unit is permanently `CAMERA_ONLY` → `DEGRADED` (**R21**); **R5** is unmitigated and night/adverse recall is **not claimed**. | Deferred to cấp sở. **Specify the monitored range first** — it decides whether any affordable radar can pass (b). |
...
> The single highest-stakes pair *was* **RQ-H1** (radar capability) and **RQ-H2** (smart sign controller): both are the foundation of the system's core safety guarantees, and both are **hardware choices**, not software ones.
>
> **As of 2026-07-10 RQ-H1 is closed as _not procured_** ([ADR-0001](adr/ADR-0001-sensing-modality.md) Rejected; consequences accepted as R5 / R20 / R21). **That leaves RQ-H2 as the only safety-bearing hardware requirement still open in this phase — and it is unmet.** The sign controller must **blank when a fresh authenticated assertion stops arriving**, whatever the bearer (UART, WiFi or LoRa — the dead-man's switch is a property of the *protocol*, not the *bearer*). A latching sign fails **dangerous** on exactly the power-loss scenario this project treats as its dominant failure mode.
