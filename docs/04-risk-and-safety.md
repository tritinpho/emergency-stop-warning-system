...
|--------------|--------|--------------|-----------------|
| Camera dead / frozen frames | Loss of class + image-ROI geometry → **cannot confirm a _new_ stop** | Frame-staleness watchdog; per-sensor health check | **RADAR-ONLY = BLIND-TO-NEW** ([ADR-0009 §B](adr/ADR-0009-failsafe-placement-and-degraded-modes.md)): may *hold* an already-confirmed track briefly via radar, but **cannot initiate** → **critical alert**, *not* a benign "degraded run"; both sensors down → SAFE STATE |
