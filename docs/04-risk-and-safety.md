...
- guarantee detection beyond the validated envelope. **Night, rain and fog lie outside it.**
  [ADR-0001](adr/ADR-0001-sensing-modality.md) (camera + radar) is **Rejected for this phase** and the
  build is **camera-only**, so the adverse-condition claim is **not made at all** — not merely unproven
  (R5, unmitigated);
- **hold a warning through an occlusion.** With no radar to corroborate, a confirmed vehicle hidden by
  a through-lane truck for longer than `T_hold` (default 10 s) is **cleared** — the sign blanks while
  the vehicle is still stopped. The clear is *loud* (the operator is alerted), so this is an
  operator-in-the-loop gap, not a silent one (R20);
- **report its own health informatively.** A camera-only unit sits permanently in `CAMERA_ONLY` mode
  and therefore shows a permanent `DEGRADED` posture. In this phase `DEGRADED` means *"radar was never
  fitted"*, **not** *"something has failed"* (R21).
...
| R5 | **Adverse-condition blindness** — night/rain/fog defeats the camera | 4 | 4 | 16 | **Unmitigated in this phase — accepted residual.** Radar was the only control and is **[Rejected 2026-07-10](adr/ADR-0001-sensing-modality.md)**; the build is camera-only. Night/rain/fog recall is **not claimed** — neither measured nor asserted from synthetic radar. Degraded-mode alerting remains. Mitigation deferred to the cấp sở field project. |
...
| R20 | **Occlusion clears the warning (camera-only build)** — with [ADR-0001](adr/ADR-0001-sensing-modality.md) Rejected there is no radar corroboration, so `corr` is never true: `WARN_HOLD` / `CAMERA_OCCLUDED_DEGRADED` are **unreachable**, and a confirmed vehicle hidden by a through-lane truck is **cleared at `T_hold` (default 10 s)** — the sign blanks while the vehicle is still stopped | 3 | 4 | 12 | **Accepted for this phase**; stated as a limit of protection (§0). The clear is *loud* (operator alerted, event logged), so the residual is **operator-in-the-loop, not silent**. Radar corroboration deferred to cấp sở. Do **not** substitute a camera-only "hold" without first deciding the cry-wolf trade — an uncorroborated hold is a guess, and R14 already shows this project prefers a quiet clear to a false assert. |
| R21 | **Permanent DEGRADED posture** — a camera-only unit never reaches `FULL` sensing mode, so the mode-keyed operator alert reports `DEGRADED` on every tick; an indicator that is always on carries no information and trains the operator to ignore it (a standing-alarm variant of R17) | 3 | 3 | 9 | **Accepted for this phase.** Bench scope has no staffed operator, so the exposure is deferred rather than live. Before any staffed deployment, re-key severity so *camera-only-by-design* is distinguished from *camera-dead*. Until then, `DEGRADED` means "radar was never fitted", **not** "something has failed". |

**Top exposures to design against first:** R5 (adverse-condition blindness), R1 (silent miss), R4
(placement too close). R1 and R4 are addressed by load-bearing decisions already taken (ADR-0005,
doc 01 §4). **R5 is now _unmitigated_:** its only control — ADR-0001 camera+radar fusion — was
**Rejected for this phase (2026-07-10)**. R5 is carried as an accepted residual and deferred to the
cấp sở project, together with the two consequences it creates, **R20** and **R21**.
