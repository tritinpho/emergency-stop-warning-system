...
| Degraded mode — *initiate* vs *hold* | (chế độ suy giảm — khởi tạo vs. duy trì) | Những gì một thiết bị vẫn làm được khi một cảm biến bị hỏng: một thiết bị có camera bị chết có thể *duy trì* một cảnh báo đang có nhưng không thể *khởi tạo* một cảnh báo mới (**mù với sự cố mới**). Phần *duy trì* chính là **trạng thái giữ khi camera không xác thực được (vết)** có giới hạn (`T_degraded_max`, [ADR-0013](adr/ADR-0013-degraded-hold-unification.vi.md)); toàn bộ **ma trận trạng thái cảnh báo × chế độ cảm biến** nằm ở [tài liệu 02 §4](02-system-architecture.vi.md#4-máy-trạng-thái-phát-hiệncảnh-báo). |
...
| `T_degraded_max` — cận thời gian giữ suy giảm | (cận thời gian suy giảm) | Cận thời gian cứng cho `CAMERA_OCCLUDED_DEGRADED` (camera không xác thực được vết — che khuất HOẶC lỗi — chỉ radar chứng thực); quá hạn này máy buộc một định đoạt **lớn tiếng** thay vì giữ bảng BẬT vô hạn dựa trên một tín hiệu radar không kiểm chứng được ([ADR-0009 §C](adr/ADR-0009-failsafe-placement-and-degraded-modes.vi.md)); giới hạn này không phụ thuộc nguyên nhân ([ADR-0013](adr/ADR-0013-degraded-hold-unification.vi.md)), khép lại cả đường stale-ON do che khuất kéo dài lẫn đường giữ do lỗi camera (RADAR-ONLY). |
| Quy trình vận hành (ConOps) / quản lý cảnh báo của người trực | (quy trình vận hành & quản lý cảnh báo) | Đường phản hồi của người trực — có nhân sự, có giới hạn — mà "báo động lớn khi sự cố" phụ thuộc vào: hành động kỳ vọng theo từng điều kiện, mức nghiêm trọng, thời gian phản hồi mục tiêu, gộp trùng/ưu tiên cảnh báo, leo thang lại khi không xác nhận ([ADR-0011](adr/ADR-0011-operator-concept-and-alarm-management.vi.md), NFR-15). |
| Mô hình mối đe dọa | (mô hình mối đe dọa) | Tập hợp tài sản, tác nhân và bề mặt tấn công được liệt kê (liên kết biển báo, ghi đè, cấu hình/OTA, vô hiệu hóa cảm biến) mà tuyên bố "không thể giả mạo" của NFR-09 được **giới hạn phạm vi** vào đó ([ADR-0012](adr/ADR-0012-security-and-threat-model.vi.md)). |
| Kích hoạt theo hiện diện (người đi bộ) | (kích hoạt theo hiện diện) | Cảnh báo cho **người** được kích hoạt theo *hiện diện* trong/cạnh ROI (có khử dội), **không** theo cổng tĩnh tại (speed gate) mà một người đang đi bộ sẽ không thỏa ([ADR-0003](adr/ADR-0003-detection-algorithm.vi.md)). |
