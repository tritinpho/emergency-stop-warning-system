...
| `T_degraded_max` — cận thời gian giữ suy giảm | (cận thời gian suy giảm) | Cận thời gian cứng cho `CAMERA_OCCLUDED_DEGRADED` (camera bị che, chỉ radar chứng thực); quá hạn này máy buộc một định đoạt **lớn tiếng** thay vì giữ bảng BẬT vô hạn dựa trên một tín hiệu radar không kiểm chứng được ([ADR-0009 §C](adr/ADR-0009-failsafe-placement-and-degraded-modes.vi.md)); đóng lại đường stale-ON duy nhất mà watchdog không chạm tới. |
