...
|----|---------------------|--------|---------------------------------|-------------------|
| ~~**RQ-H1**~~ | **ĐÃ KHÉP LẠI — KHÔNG MUA (2026-07-10).** Giai đoạn này **chỉ dùng camera**. Giữ lại làm đặc tả cho **cấp sở**, không phải một yêu cầu đang có hiệu lực: ~~radar phát hiện được một phương tiện *đứng yên* trong nhiễu nền ven đường (a) _và_ phân giải được làn dừng khẩn cấp với làn thông hành liền kề ở cự ly giám sát (b) — một mô-đun đánh giá radar tạo ảnh / HRR FMCW, **chứ không phải** một thiết bị báo hiện diện thông thường.~~ | [ADR-0001](adr/ADR-0001-sensing-modality.vi.md) **Bác bỏ** | **Đã xảy ra, và được chấp nhận:** khoảng giữ-khi-che-khuất không bao giờ kích hoạt (**R20**); thiết bị vĩnh viễn ở `CAMERA_ONLY` → `DEGRADED` (**R21**); **R5** không còn giảm thiểu và khả năng phát hiện ban đêm/bất lợi **không được tuyên bố**. | Hoãn sang cấp sở. **Xác định cự ly giám sát trước** — nó quyết định liệu có radar nào trong tầm ngân sách vượt qua được (b) hay không. |
...
> Cặp có rủi ro cao nhất *từng là* **RQ-H1** (năng lực radar) và **RQ-H2** (bộ điều khiển biển báo thông minh): cả hai đều là nền tảng cho các bảo đảm an toàn cốt lõi của hệ thống, và cả hai đều là **lựa chọn phần cứng**, không phải lựa chọn phần mềm.
>
> **Từ 2026-07-10, RQ-H1 được khép lại là _không mua_** ([ADR-0001](adr/ADR-0001-sensing-modality.vi.md) đã Bác bỏ; các hệ quả được chấp nhận là R5 / R20 / R21). **Như vậy RQ-H2 là yêu cầu phần cứng mang tính an toàn duy nhất còn mở trong giai đoạn này — và nó chưa được đáp ứng.** Bộ điều khiển biển báo phải **làm trống biển khi ngừng nhận được một khẳng định mới, có xác thực**, bất kể đường truyền là gì (UART, WiFi hay LoRa — cơ chế tự ngắt an toàn là thuộc tính của *giao thức*, không phải của *đường truyền*). Một biển báo tự chốt sẽ hỏng theo hướng **nguy hiểm** đúng vào kịch bản mất nguồn mà đề tài này coi là chế độ hỏng chủ đạo.
